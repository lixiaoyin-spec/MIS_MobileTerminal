...
- **Query 参数**：`postId=<帖子ID>`  
  - 完整示例：`/api/post/detail?postId=1`  
- **成功返回（data 包含帖子信息和评论列表）**：
  ```json
  {
    "code": 200,
    "msg": "查询成功",
    "data": {
      ...
      "viewCount": 10,
      "createTime": "2025-11-26T12:00:00",
      "updateTime": "2025-11-26T12:00:00",
      "comments": [
        {
          "id": 1,
          "postId": 1,
          "nickname": "Bob",
          "content": "评论内容",
          "createTime": "2025-11-26T13:00:00"
        }
      ]
    }
  }
...
### 8. 获取帖子评论列表（可选，推荐使用帖子详情接口）
- **方法**：GET  
- **URL**：`/api/comment/list`  
- **Headers**：`Authorization: Bearer <登录返回的 token>`  
- **Query 参数**：`postId=<帖子ID>`  
  - 完整示例：`/api/comment/list?postId=1`  
- **说明**：此接口仅返回评论列表，如需同时获取帖子信息，请使用 **帖子详情接口**（`/api/post/detail`）
- **成功返回（data 为评论列表）**：
  ```json
  {
    "code": 200,
